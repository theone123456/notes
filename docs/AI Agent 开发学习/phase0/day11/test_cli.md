...
> 源文件：`phase0/day11/test_cli.py`

```python
"""Day 11 · chat_cli 自动化测试

分层：
    零 API 套件（默认运行，不烧 token）——CLI 机制验证
        pytest
    真实 API 套件（烧 token，全套约 13 次调用）——Step 4 实验数据 + Step 5 完整演练
        pytest -m api

设计要点：
    - 每个用例在 pytest 临时目录（tmp_path）里以子进程跑 chat_cli.py，
      history.json 相对运行目录落盘 -> 天然隔离，不碰 day11 真实历史
...
def run_cli(inputs, cwd, extra_env=None, timeout=120):
    """管道方式跑 chat_cli.py：inputs 逐行喂给 stdin，返回 CompletedProcess。"""
    env = {**os.environ, **_DOTENV}
    if extra_env:
        env.update(extra_env)
    return subprocess.run(
        [sys.executable, str(CHAT_CLI)],
        input="\n".join(inputs) + "\n",
        capture_output=True,
        text=True,
        cwd=cwd,
...
def test_clear_keeps_loaded_persona(tmp_path):
    """A 语义回归（readme Q3 Step 4 延伸）：resume 后 clear 保留载入历史的人设。
...
def test_wrong_key_no_crash_loop_continues(tmp_path):
    """Step 4 失败演练：错 Key 时 CLI 不崩、提示清晰、循环继续、零计费、失败轮不落盘。
...
# ---------- 真实 API 套件：Step 4 实验数据 + Step 5 完整演练 ----------
# 运行方式：pytest -m api（烧 token，全套约 13 次调用）

@pytest.mark.api
@pytest.mark.skipif(not API_READY, reason="Development/.env 缺 API_KEY")
...
def test_step5_full_walkthrough(tmp_path):
    """Step 5 完整演练：多轮聊天（记忆）-> /clear -> 新话题 -> /exit -> 重启 -> /resume -> 历史恢复。"""
    session_a = [
        f"我叫{NAME}，今年{AGE}岁",
        "我刚才说我叫什么名字？",
        "/clear",
